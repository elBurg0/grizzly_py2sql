...
class Python3Visitor(ParseTreeVisitor):
    ...
    def visitAssignment_stmt(self, ctx:Python3Parser.Assignment_stmtContext):
        ...
        var = str(ctx.NAME())
        var_type = sqlgenerator.SQLGenerator._mapTypes(str(ctx.typ().getText()), self.profile)
    ...
    def visitFor_stmt(self, ctx:Python3Parser.For_stmtContext):
        if ctx.rang():
            self.statements.append("FOR " + str(ctx.expr().NAME()) + " IN " + str(ctx.rang().expr()[0].getText()) + ".." + str(ctx.rang().expr()[1].getText()))
